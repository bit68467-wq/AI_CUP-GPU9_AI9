...
(async function(){
  ...
  if (!window.websim) {
    ...
    // Load any previously persisted collections (from the app persistence layer) to avoid resetting accounts.
    (function loadPersistedCollections(){
      const keys = {
        ...
        meta_v1: 'cup9gpu_persistent_meta_v1',
      };
      const db = { collections: {} };
      try {
        // Prefer the explicit persistent keys; fallback to a legacy single-key store if present.
        for (const [col, key] of Object.entries(keys)) {
          try {
            const raw = localStorage.getItem(key);
            if (raw) {
              const parsed = JSON.parse(raw || '[]');
              db.collections[col] = Array.isArray(parsed) ? parsed.slice() : [];
              continue;
            }
          } catch(e){}
          db.collections[col] = [];
        ...
        // fallback legacy blob (if older installations used a single key)
        if (Object.values(db.collections).every(arr => Array.isArray(arr) && arr.length === 0)) {
          try {
            const legacy = JSON.parse(localStorage.getItem('cup9gpu_db_v1') || '{}');
            if (legacy && legacy.collections) {
              for (const k of Object.keys(legacy.collections)) {
                db.collections[k] = Array.isArray(legacy.collections[k]) ? legacy.collections[k].slice() : [];
              }
            }
          } catch(e){}
        }

        window.__cup9_localmock_db = db;
      } catch(e){
        window.__cup9_localmock_db = { collections: {} };
      }
    })();
    ...
    window.websim = {
      ...
      __db: window.__cup9_localmock_db || { collections: {} },
      __saveDb(){
        try {
          // persist collections back to the specific persistent keys used by the app so data survives restarts
          const mapping = {
            user_v1: 'cup9gpu_persistent_users_v1',
            transaction_v1: 'cup9gpu_persistent_transactions_v1',
            device_v1: 'cup9gpu_persistent_devices_v1',
          ...
          for (const [col, key] of Object.entries(mapping)) {
            try {
              const arr = this.__db.collections[col] || [];
              localStorage.setItem(key, JSON.stringify(arr));
            } catch(e){}
          }
          // keep a legacy blob for compatibility
          try { localStorage.setItem('cup9gpu_db_v1', JSON.stringify(this.__db)); } catch(e){}
        } catch(e){ console.warn('persist failed', e); }
      },
      collection(name){
        this.__db.collections[name] = this.__db.collections[name] || [];
        // internal subscription storage per collection
        this.__db.collections['_subs_' + name] = this.__db.collections['_subs_' + name] || [];
        const colList = this.__db.collections[name];
        const colSubs = this.__db.collections['_subs_' + name];

        const persistAndNotify = ()=>{
          this.__saveDb();
          colSubs.forEach(s=> {
            try { s(colList.slice()); } catch(e){}
          });
        };
        return {
          async create(data){
          ...
          getList(){ return colList.slice(); },
          filter(obj){
            return {
              getList(){ return colList.filter(r=>Object.keys(obj).every(k=>r[k]===obj[k])); },
              subscribe(fn){ colSubs.push(fn); return ()=>{ const idx = colSubs.indexOf(fn); if (idx>=0) colSubs.splice(idx,1); } }
            };
          },
          subscribe(fn){ colSubs.push(fn); return ()=>{ const idx = colSubs.indexOf(fn); if (idx>=0) colSubs.splice(idx,1); } },
          async update(id,data){
            const i = colList.findIndex(x=>x.id===id);
            if (i>=0) colList[i] = {...colList[i],...data};
            persistAndNotify();
          },
          async delete(id){
            const idx = colList.findIndex(x=>x.id===id);
            if (idx>=0) colList.splice(idx,1);
            persistAndNotify();
          }
        };
      }
    ...
    return {
      ...
      async delete(id){
        try {
          ...
          // best-effort local delete: remove from cache and notify subscribers
          try {
            if (cache) {
              cache = cache.filter(x => x.id !== id);
              subs.forEach(safe => { try { safe(cache); } catch(err){} });
            }
          } catch (inner) {
            // ignore
          }
          // Do not rethrow to prevent unhandledRejection; return a failure object so callers can handle it gracefully.
  ...
  // On boot, if there is a local session persisted, restore it into in-memory session and ensure
  // a server-side session record exists so users (and admin) remain persistent across refreshes/devices.
  (function restoreSessionFromLocalStorage(){
    try {
      ...
      if (!parsed || !parsed.uid) return;

      // hydrate in-memory session for this tab immediately
      window.__cup9_session = Object.assign({
        id: parsed.id || parsed.user_id || parsed.userId || null,
        uid: parsed.uid || parsed.user_uid || parsed.id || null,
        username: parsed.username || parsed.name || parsed.email || 'user',
        email: parsed.email || null,
        is_admin: !!parsed.is_admin,
        updated_at: parsed.updated_at || new Date().toISOString()
      }, parsed);

      // attempt to persist or reconcile with server-side sessions collection so session is globally available
      (async () => {
        try {
          // try to find an existing session record matching this uid
          const existing = sessionsCol.getList().find(s => (s.uid && s.uid === window.__cup9_session.uid) || (s.user_uid && s.user_uid === window.__cup9_session.uid));
          if (existing && existing.id) {
            // update remote/session record so token/metadata stays fresh
            try {
              await sessionsCol.update && sessionsCol.update(existing.id, {
                user_id: window.__cup9_session.id,
                uid: window.__cup9_session.uid,
                username: window.__cup9_session.username,
                email: window.__cup9_session.email,
                updated_at: new Date().toISOString()
              });
              window.__cup9_session.session_id = existing.id;
            } catch(e){
              // fallback: ignore update errors
            }
          } else {
            // create a new persistent session record
            try {
              const rec = await sessionsCol.create({
                user_id: window.__cup9_session.id,
                uid: window.__cup9_session.uid,
                username: window.__cup9_session.username,
                email: window.__cup9_session.email,
                is_admin: !!window.__cup9_session.is_admin,
                created_at: new Date().toISOString(),
                updated_at: new Date().toISOString()
              });
              if (rec && rec.id) window.__cup9_session.session_id = rec.id;
            } catch(e){
              // ignore create errors (offline/local mode)
            }
          }
        } catch (e) {
          console.warn('restore session persistence failed', e);
        } finally {
          // persist the normalized session back to localStorage (keeps format consistent)
          try { localStorage.setItem(SESSION_KEY, JSON.stringify(window.__cup9_session)); } catch(e){}
        }
      })();
    } catch (e) {
  ...
  function adminPage(){
    ...
    // Render pending helper (unchanged core logic)
    function renderPending(filterUid){
      ...
      list.forEach(t=>{
        ...
        actions.appendChild(sendOtp); actions.appendChild(confirmNow); actions.appendChild(inspect);
        right.appendChild(actions);
        row.appendChild(right);
    ...
    uidBtn.onclick = ()=> {
      ...
      creditBtn.onclick = async ()=>{
        ...
        // signal admin action
        localStorage.setItem('cup9gpu_last_admin_action', JSON.stringify({ ts: Date.now(), action: 'credit_deposit', id: q, amount: val, by: session.username || 'admin' }));
        window.dispatchEvent(new CustomEvent('cup9gpu_admin_action', { detail:{ type:'credit_deposit', id:q, amount: val } }));
      ...
      creditEarningBtn.onclick = async ()=>{
        ...
        localStorage.setItem('cup9gpu_last_admin_action', JSON.stringify({ ts: Date.now(), action: 'credit_earning', id: q, amount: val, by: session.username || 'admin' }));
        window.dispatchEvent(new CustomEvent('cup9gpu_admin_action', { detail:{ type:'credit_earning', id:q, amount: val } }));
      ...
      debitBtn.onclick = async ()=>{
        ...
        localStorage.setItem('cup9gpu_last_admin_action', JSON.stringify({ ts: Date.now(), action: 'debit_purchase', id: q, amount: val, by: session.username || 'admin' }));
        window.dispatchEvent(new CustomEvent('cup9gpu_admin_action', { detail:{ type:'debit_purchase', id:q, amount: val } }));
